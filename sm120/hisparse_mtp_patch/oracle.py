...
def child(mode: str, a) -> dict:
    ...
    pf_stats = getattr(hrt, "_SUFFIX_HISPARSE_PREFETCH_STATS", None)
    pf_base = dict(pf_stats) if pf_stats else {}
    layout = None
    if hisparse:
    ...
    if a.multi:
        ...
        return {
            ...
            "prefetch": {key: v - pf_base.get(key, 0) for key, v in pf_stats.items()}
            if pf_stats else None,
